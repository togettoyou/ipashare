...
type AppleDevice struct {
	...
	AccountIss string    `gorm:"not null;column:account_iss;comment:绑定的开发者账号Iss"`
	Udid       string    `gorm:"not null;column:udid;comment:UDID设备标识"`
	DeviceId   string    `gorm:"not null;column:deviceId;comment:设备在开发者后台的id"`
